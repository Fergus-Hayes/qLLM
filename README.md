...
**Memory:** a gradient (`gradient` / `explicit+gradient`) disentangling of a deep
circuit is the memory-hungry case -- the big `q_proj`/`o_proj` layers at high `D`
on many workers can exhaust RAM and a worker gets OOM-killed. Two things keep the
sweep within budget:

* The gradient path is **gradient-checkpointed**: the circuit is trained by
  recomputing intra-segment activations in backward rather than storing one per
  gate, so training memory grows as `~sqrt(depth)` instead of linearly. This alone
  turns a `D = 256`, `k = 4` disentangling from an out-of-memory kill into a job
  that fits in a few GB (the result is bit-identical -- it only trades a little
  recompute for the memory). Explicit-only training is already flat in `D`.
* If a pool still can't fit, the sweep **steps the worker count down**
  (`6 -> 3 -> 2 -> 1`) and retries only the jobs that didn't come back, reaping
  each pool fully before the next so a dead worker's memory can't pile onto the
  retry. Anything even a single worker can't deliver -- or a pool that never
  starts (e.g. the child can't import `qllm`) -- is finished in-process. The sweep
...
# Refine to directly minimize the reconstruction error at each served chi'
python hybridize.py --gate-sizes 2 --circuit-depths 0 1 2 4 --chi 1 2 4 8 \
    --disentangle-optimizer explicit+gradient --fast-gradient \
    --gradient-objective relative-error --disentangle-target-per-chi \
    --disentangle-gd-steps 200 --disentangle-gd-lr 0.03
```
